(function ($) {
	...
	$('select.nice-select-list').niceSelect();
	...
	const API_SERVER_URL = window.location.protocol + "//" + window.location.host + "/api/v1/posts/";
	$('body').delegate('#submit_post', 'click', function(e){
		let t = $(this);
		let form = $('#create-post-form');
		let formData = form.serialize();
		let url = API_SERVER_URL+'create-post';
		console.log(url);
		$.ajax({
			url: url,
			type: 'POST',
